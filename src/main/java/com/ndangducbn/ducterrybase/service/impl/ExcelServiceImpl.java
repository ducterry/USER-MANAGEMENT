...
public class ExcelServiceImpl implements ExcelService {
    ...
    public ExcelServiceImpl(Convert2DTO convert2DTO, UserRepository userRepository, ModelMapper modelMapper) {
    ...
    public String exportUserToExcel() {
        try {
             ...
             */
            String path = PathConstant.EXCEL_PATH_EXPORT_SAVED;
            ...
            try {
                path = path +"/DANH_SACH_TAI_KHOAN"+ DateUtil.dateToString(ETypeDateFormat.DATE_FORMAT_FULL, new Date()) + ".xlsx";
    ...
    @Override
    public String downloadTemplates() {
        try {
            log.debug(PREFIX + "downloadTemplates =>");

            /*
                01. Tạo thư mục Export File
             */
            String path = PathConstant.EXCEL_PATH_DOWNLOAD_SAVED;
            File dir = new File(path);
            if (!dir.exists()) {
                dir.mkdirs();
            }
            ...
            try {
                path = path +"/DANH_SACH_TAI_KHOAN"+ DateUtil.dateToString(ETypeDateFormat.DATE_FORMAT_FULL, new Date()) + ".xlsx";
                excelFile = new ClassPathResource(PathConstant.EXCEL_DOWNLOAD_TEMPLATES).getInputStream();
                workbook = new XSSFWorkbook(excelFile);
            } catch (Exception e) {
                log.error(PREFIX + "Export Excel Exception => {}", JSONFactory.toString(e.getMessage()));
            }
